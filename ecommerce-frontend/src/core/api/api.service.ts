...
import { URL_API_ENDPOINT } from "../constant/app.constant"
import { getCookies } from "@/helpers/cookie.helper"

const client = axios.create({
    baseURL: `${URL_API_ENDPOINT}/api/v1`,
    headers: {
...
class ApiService {
    static get(path = '', params = {}, optionalHeader = {}, urlPathVariable: Record<string, string> = {}): AxiosPromise {
        const interpolatedPath = Object.keys(urlPathVariable).reduce((acc, key) => {
            return acc.replace(`:${key}`, urlPathVariable[key])
        }, path)
    ...
    static async getDownloadFile(path = '', optionalHeader = {}): Promise<File> {
    ...
    static put(path = '', data = {}, params = {}, urlPathVariable: Record<string, string> = {}): AxiosPromise {
        const interpolatedPath = Object.keys(urlPathVariable).reduce((acc, key) => {
            return acc.replace(`:${key}`, urlPathVariable[key])
        }, path)
    ...
    static delete(path = '', data = {}, urlPathVariable: Record<string, string> = {}): AxiosPromise {
        const interpolatedPath = Object.keys(urlPathVariable).reduce((acc, key) => {
            return acc.replace(`:${key}`, urlPathVariable[key])
        }, path)
